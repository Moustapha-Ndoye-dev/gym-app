...
export const getAllActivities = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
...
export const createActivity = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const id = await ActivityModel.create({ ...req.body, gymId: req.user.gymId });
...
export const updateActivity = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    if (changes === 0) return res.status(404).json({ message: 'Activité introuvable' });
