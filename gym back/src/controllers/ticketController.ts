...
export const getAllTickets = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
...
export const deleteTicket = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    const id = parseInt(req.params.id as string);
    const changes = await TicketModel.delete(id, req.user.gymId);
    if (changes === 0) return res.status(404).json({ message: 'Ticket introuvable' });
