...
export const createUser = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const id = await UserModel.createUser(username, password, role, gymId || 1);
...
export const updateUser = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const changes = await UserModel.update(parseInt(req.params.id as string), username, role, password);
